...
def main():
    ...
    # Calculate ranks
    eval_data['jina_reranker_v2_base_multilingual_rank'] = model.compute_score([[eval_data.cands.iloc[ix], eval_data.refs.iloc[ix]] for ix in range(len(eval_data.cands))], 
                                                                               batch_size=128)
    
    # implode dataset to old format
    eval_data = (eval_data.groupby('id')
      .agg({'jina_reranker_v2_base_multilingual_rank': lambda x: x.tolist()})
      .reset_index())
    data = data.set_index('id')
    eval_data = eval_data.set_index('id')
    data = data.merge(eval_data.jina_reranker_v2_base_multilingual_rank, on='id').reset_index().drop(columns='id')
    
    # Store the results
    save_file = args.data_file.replace(".jsonl", "_jina_reranker_v2_base_multilingual_rank.jsonl")
